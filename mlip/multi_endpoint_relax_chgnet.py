...
def load_calculator():
    """CHGNet v0.4.2 calculator."""
    from chgnet.model.dynamics import CHGNetCalculator
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[load] CHGNet on {device}", flush=True)
...
def main():
    ...
    print(f"=== Multi-endpoint CHGNet relax (cross-check) ===", flush=True)
    ...
    with open(out_dir / "DONE_chgnet_multi_endpoint", "w") as f:
        f.write(f"Relaxed {len(successful)}/{manifest['n_candidates']} in "
                f"{time.time()-t_global:.0f}s\n")
        for s in successful[:5]:
            f.write(f"  {s['candidate_id']}: E={s['E_final_eV']:.4f}\n")

    print(f"\n[DONE] {len(successful)}/{manifest['n_candidates']}", flush=True)
    return 0
